...
constexpr gfx::Window::Extent kWidowExtent{.width = kWindowWidth, .height = kWindowHeight};
...
TEST(ArcballTest, GetNormalizedViewPositionConvertsScreenCoordiantesToNormalizedXyViewCoordiantes) {
  ...
  static_assert(GetNormalizedViewPosition(kTopLeftCorner, kWidowExtent) == glm::vec2{-1.0f, 1.0f});

  static constexpr glm::vec2 kBottomLeftCorner{0.0f, kWindowHeight};
  static_assert(GetNormalizedViewPosition(kBottomLeftCorner, kWidowExtent) == glm::vec2{-1.0f, -1.0f});

  static constexpr glm::vec2 kBottomRightCorner{kWindowWidth, kWindowHeight};
  static_assert(GetNormalizedViewPosition(kBottomRightCorner, kWidowExtent) == glm::vec2{1.0f, -1.0f});

  static constexpr glm::vec2 kTopRightCorner{kWindowWidth, 0.0f};
  static_assert(GetNormalizedViewPosition(kTopRightCorner, kWidowExtent) == glm::vec2{1.0f, 1.0f});

  static constexpr glm::vec2 kCenter{kWindowWidth / 2.0f, kWindowHeight / 2.0f};
  static_assert(GetNormalizedViewPosition(kCenter, kWidowExtent) == glm::vec2{0.0f, 0.0f});
}
